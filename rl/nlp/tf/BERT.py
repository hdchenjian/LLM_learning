...
def train():
    ...
    for epoch in range(500):
        for batch_idx, batch in enumerate(loader):
            ...
            seqs, segs, seqs_, nsp_labels, loss_mask = seqs.type(torch.LongTensor).to(device), segs.type(torch.LongTensor).to(device), \
                seqs_.type(torch.LongTensor).to(device),nsp_labels.to(device),loss_mask.to(device)
